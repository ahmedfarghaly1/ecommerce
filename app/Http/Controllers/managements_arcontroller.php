...
class managements_arcontroller extends Controller {
	...
	public function request_ar($value,$id) {



		$mains =DB::table('direct_meet_request_ar')->find($id) ;
		$mains =DB::table('main_meet_request_ar')->find($id) ;
		$mains =DB::table('rent_meet_request_ar')->find($id) ;
		$mains =DB::table('fin_meet_request_ar')->find($id) ;
		$mains =DB::table('t_request_ar')->find($id) ;
		$mains =DB::table('col_meet_request_ar')->find($id) ;
		$mains =DB::table('legal_meet_request_ar')->find($id) ;


		if ($value == 'المدير العام') {
			return view('request.manager_meet_request_ar',compact('mains'));
		} elseif ($value == 'إدارة الصيانة') {
			return view('request.main_request_ar',compact('mains'));
		} elseif ($value == 'إدارة التأجير') {
			return view('request.rent_request_ar',compact('mains'));
		} elseif ($value == 'الادارة المالية') {
			return view('request.fin_request_ar',compact('mains'));
		} elseif ($value == 'إدارة التحصيل') {
			return view('request.t_request_ar',compact('mains'));
		} elseif ($value == 'إدارة المشاريع') {
			return view('request.project_request_ar',compact('mains'));
		} elseif ($value == 'الإدارة القانونية') {
			return view('request.low_request_ar',compact('mains'));
		}
	}
